...
4. UI改善: 詳細モーダル（対応中）、カタカナ表示修正（下記13で正体判明・部分対応済み）
...
13. 静岡・長野・岐阜・神奈川のcp932/UTF-8二重デコード文字化け → 可逆な1,309件は復元・push済み。
    復元不可の290件（静岡229・長野48・岐阜12・神奈川1）は再スクレイプが必要。
    特に**scrape_shizuoka.pyがリポジトリから消失しており新規作成が必要**（詳細はNOTES_20260704.md参照）
